...
export function buildDraftArticle(options: ArticleOptions): Record<string, unknown> {
  ...
  if (options.articleType === "newspic") {
    ...
    article = {
      ...
      title: options.title,
      content: htmlToPlainText(options.content),
...
async function publishArticlesToDraft(
  ...
  const res = await client(url, {
    ...
    body: JSON.stringify({ articles: articles.map(buildDraftArticle) }),
  });
...
async function publishToDraft(
  ...
  return await publishArticlesToDraft([options], accessToken, client);
}
...
function truncateDigest(value: string): string {
  ...
  const truncated = value.slice(0, 117);
...
export function loadMultiManifest(inputPath?: string): MultiManifest | undefined {
  if (!inputPath) return undefined;
  const manifestPath = path.resolve(inputPath);
  if (!fs.existsSync(manifestPath)) {
    throw new Error(`Multi-article manifest not found: ${manifestPath}`);
  }
  const raw = JSON.parse(fs.readFileSync(manifestPath, "utf-8")) as {
    articles?: Array<{ path?: unknown; title?: unknown }>;
  };
  if (!Array.isArray(raw.articles) || raw.articles.length === 0) {
    throw new Error("Multi-article manifest must contain a non-empty articles array");
  }
  const manifestDir = path.dirname(manifestPath);
  const articles = raw.articles.map((entry, index) => {
    if (typeof entry.path !== "string" || !entry.path.trim()) {
      throw new Error(`Multi-article manifest entry ${index + 1} is missing path`);
    }
    const articlePath = path.isAbsolute(entry.path)
      ? entry.path
      : path.resolve(manifestDir, entry.path);
    if (!fs.existsSync(articlePath)) {
      throw new Error(`Multi-article source not found: ${articlePath}`);
    }
    return {
      path: articlePath,
  ...
  return { path: manifestPath, articles };
}
...
Options:
  ...
  --multi-manifest <path>  Publish all manifest articles in one draft
...
async function main(): Promise<void> {
  ...
  const multiManifest = loadMultiManifest(args.multiManifest);
  ...
  if (args.dryRun) {
    console.log(JSON.stringify({
      ...
      multiManifest: multiManifest?.path,
      articleCount: multiManifest?.articles.length ?? 1,
  ...
  const publishWith = async (client: WechatClient): Promise<void> => {
    ...
    if (multiManifest) {
      ...
      for (const entry of multiManifest.articles) {
        ...
        let entrySourceUrl = "";
        let entryHtml = "";
        let entryImageMediaIds: string[] = [];

        if (entryPath === filePath) {
          entryTitle = entry.title || title;
          entryDigest = digest;
          entrySourceUrl = sourceUrl;
          entryHtml = htmlContent;
          entryImageMediaIds = imageMediaIds;
        } else {
          const entryContent = fs.readFileSync(entryPath, "utf-8");
          const entryParsed = parseFrontmatter(entryContent);
          entryTitle = entry.title || entryParsed.frontmatter.title || "";
          entryAuthor = entryAuthor || entryParsed.frontmatter.author || "";
          entryDigest = entryParsed.frontmatter.digest
            || entryParsed.frontmatter.summary
            || entryParsed.frontmatter.description
            || "";
          entrySourceUrl = entryParsed.frontmatter.sourceUrl
            || entryParsed.frontmatter.contentSourceUrl
            || entryParsed.frontmatter.content_source_url
            || "";

          let entryImages: ImageInfo[] = [];
          if (entryPath.toLowerCase().endsWith(".html")) {
            entryHtml = extractHtmlContent(entryPath);
            entryTitle = entryTitle || extractHtmlTitle(entryContent);
          } else {
            const heading = entryParsed.body.match(/^#\s+(.+)$/m);
            entryTitle = entryTitle || heading?.[1] || path.basename(entryPath, path.extname(entryPath));
            const rendered = renderMarkdownWithPlaceholders(
              entryPath,
              args.theme,
              args.color,
              args.citeStatus,
            ...
            entryHtml = extractHtmlContent(rendered.htmlPath);
            entryImages = rendered.contentImages;
            entryAuthor = entryAuthor || rendered.author;
            entryDigest = entryDigest || rendered.summary;
          }

          const uploaded = await uploadImagesInHtml(
            entryHtml,
            accessToken,
          ...
          entryHtml = uploaded.html;
          entryImageMediaIds = uploaded.imageMediaIds;
        }

        articles.push({
          title: entryTitle,
          author: entryAuthor || undefined,
          digest: entryDigest ? truncateDigest(entryDigest) : undefined,
          content: entryHtml,
          thumbMediaId,
          articleType: args.articleType,
          contentSourceUrl: entrySourceUrl || undefined,
          imageMediaIds: args.articleType === "newspic" ? entryImageMediaIds : undefined,
          needOpenComment: resolved.need_open_comment,
          onlyFansCanComment: resolved.only_fans_can_comment,
        });
      }

      console.error(`[wechat-api] Publishing multi-article draft... (${articles.length} articles)`);
      const result = await publishArticlesToDraft(articles, accessToken, client);
      console.log(JSON.stringify({
        success: true,
