...
2. **完整阅读 [references/initial-screening-policy.md](references/initial-screening-policy.md)**。只根据 `list.json` 的标题做保守粗筛：为每篇论文输出 `download` 或 `skip` 和简短原因；不评分、不分 taxonomy、不读取 PDF。写入仓库外临时 JSON，执行 `screening commit --date ... --input ...`，再执行 `screening validate`。未完成该阶段不得下载。
...
6. **完整阅读 [references/publication-policy.md](references/publication-policy.md)**，按其稿件结构和文案标准生成 `.work/yyyyMMdd/copy.json`。`title` 必须是对 `list.json` 英文原标题的简洁中文翻译；正文始终由 CLI 展示英文原标题，二者不得混写或用摘要替代标题。发布稿总收录量是 50 篇硬上限，CLI 按总分降序、arXiv ID 稳定选择；完整 `editorial.json` 不受此上限影响。发布文案可以逐字段语义缩减，但不得改写或删减 `editorial.json`；精选只取入选稿中 `>=7` 分的前 4 篇，不足时不得补位。其余全览文章每篇最多 30 篇论文，由脚本自动拆分；真实微信公众号渲染字符数必须逐篇不超过 100,000。
